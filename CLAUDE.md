...
Reeve는 AI 기반 차량 제조사/모델 자동 식별 시스템으로, 두 개의 독립적인 FastAPI 서비스로 구성됩니다.
...
# 프로덕션 시작 (qdrant + identifier + redis + celery-worker + ollama + llamafactory)
...
| 파일 | 용도 |
|------|------|
| `docker-compose.yml` | 프로덕션 베이스 (qdrant, identifier, redis, celery-worker, ollama, llamafactory) |
| `docker-compose.dev.yml` | 개발 오버라이드 (mysql, studio 추가, 소스 bind-mount) |
| `docker-compose.gpu.yml` | NVIDIA GPU 설정 |
| `docker-compose.mac.yml` | macOS CPU 모드 |
...
두 서비스 모두 Pydantic Settings로 `docker/.env` 로드.
