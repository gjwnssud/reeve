...
docker compose -f docker/docker-compose.yml -f docker/docker-compose.dev.yml -f docker/docker-compose.gpu.yml -f docker/docker-compose.ssl.yml up -d
```
...
| `MAX_UPLOAD_SIZE` | `5242880` | 단일 이미지 최대 크기 (바이트, 기본 5MB) |
| `ALLOWED_EXTENSIONS` | `jpg,jpeg,png,webp` | 허용 확장자 |
| `STUDIO_PORT` | `8000` | Studio 호스트 포트 |
...
**컨테이너 메모리 한도** (`.env`에서 재정의, 미설정 시 기본값 사용):

| 변수 | 기본값 | 설명 |
|------|--------|------|
| `STUDIO_MEMORY_LIMIT` | `4G` | Studio 컨테이너 메모리 한도 |
| `IDENTIFIER_MEMORY_LIMIT` | `4G` | Identifier 컨테이너 메모리 한도 |
| `TRAINER_MEMORY_LIMIT` | `12G` | Trainer 컨테이너 메모리 한도 |
| `MYSQL_MEMORY_LIMIT` | `2G` | MySQL 컨테이너 메모리 한도 |
| `OLLAMA_MEMORY_LIMIT` | `8G` | Ollama 컨테이너 메모리 한도 |
| `CELERY_MEMORY_LIMIT` | `4G` | Celery Worker 컨테이너 메모리 한도 |
| `REDIS_MEMORY_LIMIT` | `512M` | Redis 컨테이너 메모리 한도 |

> DGX Spark 등 통합 메모리 환경에서 대형 모델 학습 시 `TRAINER_MEMORY_LIMIT=50G` 이상 권장 (기본 12G로는 unfreeze 후 OOM Kill 가능).
...
| `OPENAI_MODEL` | `gpt-5.4-mini` | OpenAI 비전 모델 |
