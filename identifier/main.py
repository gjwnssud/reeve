...
2. **CLIP** 모델(ViT-B/32)로 차량 이미지를 512차원 벡터로 변환
...
| 범위 | 의미 |
|------|------|
| 0.80 이상 | 높은 신뢰도 — `status: identified` |
| 0.50 ~ 0.79 | 낮은 신뢰도 — `status: uncertain`, 수동 확인 권장 |
| 0.50 미만 | 판별 불가 — `status: uncertain` 또는 `no_data` |
...
> 권장 조건을 충족하지 않는 이미지는 낮은 신뢰도(confidence)로 반환되거나 `uncertain` 처리될 수 있습니다.
"""
...
app = FastAPI(
    ...
    openapi_tags=[
        {
            "name": "Health",
            "description": "서비스 컴포넌트(CLIP, YOLO, Qdrant) 상태 확인",
        },
        {
            "name": "Async",
            "description": (
...
async def health_check():
    """
    CLIP 모델, YOLO 모델, Qdrant 연결 상태를 한번에 확인합니다.

    모니터링 시스템의 헬스체크 엔드포인트 또는 서비스 점검 용도로 사용하세요.

    **응답 필드**
    - `status`: `healthy` (정상) / `degraded` (Qdrant 오류, 판별 불가) / `unhealthy` (CLIP 미로드, 서비스 불가)
    - `clip_model`: CLIP 모델 로드 상태
...
async def identify_vehicle(
    ...
    **처리 흐름**
    1. `bbox` 지정 시: 해당 영역을 크롭하여 CLIP 임베딩 생성
    ...
    **`status` 필드 해석**
    ...
    | 값 | 의미 | 권장 대응 |
    |----|------|----------|
    | `identified` | 판별 성공 (confidence ≥ 0.8) | 결과를 그대로 사용 |
    | `uncertain` | 신뢰도 부족 | 수동 확인 또는 고화질 재촬영 |
    | `no_data` | 학습 데이터 없음 | 관리자에게 학습 데이터 추가 요청 |
...
async def identify_batch(
    ...
    YOLO26 감지 → CLIP 임베딩 → Qdrant 검색의 각 단계를 배치로 병렬 처리하므로
