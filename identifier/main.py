...
이미지 업로드 → YOLO 감지 → EfficientNetV2-M 분류 → 제조사/모델 판별
"""
import asyncio
import contextvars
import logging
...
# RequestIdFilter를 root logger가 아닌 각 handler에 부착.
# Python logging의 callHandlers()는 propagate로 올라온 레코드에 대해
# 부모 로거의 filter를 거치지 않고 handler를 직접 호출하므로,
# filter가 root logger에만 있으면 자식 로거(uvicorn 등)의 레코드는
# request_id가 주입되지 않아 RotatingFileHandler.shouldRollover() 에서 ValueError 발생.
...
logging.basicConfig(level=log_level, format=log_format, force=True, handlers=_log_handlers)
...
2. **EfficientNetV2-M** 분류기로 차량 종류 판별 (softmax 분류)
3. 신뢰도 임계값 기준으로 `identified` / `low_confidence` 결정
...
| 범위 | 의미 |
|------|------|
| 0.80 이상 | 충분한 신뢰도 — `status: identified` |
| 0.80 미만 | 후보 있음, 확신 부족 — `status: low_confidence`, 수동 확인 권장 |
| 분류기 미로드/예외 | `status: no_match` |

> `CLASSIFIER_CONFIDENCE_THRESHOLD` 환경변수로 임계값을 조정할 수 있습니다 (기본값: `0.80`).
...
app = FastAPI(
    ...
    openapi_tags=[
        {
            "name": "Health",
            "description": "서비스 컴포넌트(EfficientNetV2-M, YOLO) 상태 확인",
        },
        {
            "name": "Async",
            "description": (
...
async def health_check():
    ...
    **응답 필드**
    - `status`: `healthy` (정상) / `unhealthy` (분류기 미로드, 서비스 불가)
    - `efficientnet_classifier`: EfficientNetV2-M 분류기 로드 상태
...
async def identify_vehicle(
    ...
    **처리 흐름**
    1. `bbox` 지정 시: 해당 영역을 크롭
    2. `bbox` 미지정 시: YOLO26으로 이미지 내 가장 큰 차량 자동 감지 후 크롭
    3. EfficientNetV2-M 분류기로 제조사/모델 판별
...
async def identify_batch(
    ...
    YOLO26 감지 → EfficientNetV2-M 분류의 각 단계를 배치로 처리하므로
    이미지를 1장씩 반복 호출하는 것보다 효율적입니다.
